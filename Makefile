...
database-setup:
	PGPASSWORD=123456 psql -U threerolltwo_admin -h db -f sql/base.sql postgres -c \
		"SELECT pg_terminate_backend(pg_stat_activity.pid) \
		FROM pg_stat_activity \
		WHERE pg_stat_activity.datname = 'threerolltwo_com' \
		AND pid <> pg_backend_pid()"

	PGPASSWORD=123456 psql -U threerolltwo_admin -h db postgres -c \
		"DROP DATABASE IF EXISTS threerolltwo_com;"

	PGPASSWORD=123456 psql -U threerolltwo_admin -h db postgres -c \
		"CREATE DATABASE threerolltwo_com;"

	PGPASSWORD=123456 psql -U threerolltwo_admin -h db -f sql/base.sql threerolltwo_com
